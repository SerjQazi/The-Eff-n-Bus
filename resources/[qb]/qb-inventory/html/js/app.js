...
$(document).on("click", "#item-use", function (e) {
  if (contextMenuSelectedItem && ItemInventory) {
    ...
    console.log(
      "contextMenuSelectedItem or ItemInventory is not set correctly",
    );
  }
});
...
$(document).click(function (event) {
  ...
  if (
    !rightClickMenu.is(event.target) &&
    rightClickMenu.has(event.target).length === 0
  ) {
...
function FormatItemInfo(itemData, dom) {
  ...
  // ID Cards
  var csnPart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-address-card'></i> " +
    itemData.info.citizenid +
    "</b></span>";
  var firstNamePart =
    "<span style=\"background-color: #1a1c1c; padding: 5px; margin-left: 5px;\"><b> <i class='fas fa-address-card'></i> " +
  ...
  // Harness
  var harnessUsePart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-user-tag'></i> " +
    itemData.info.uses +
    " uses left." +
    "</b></span>";

  // Weapons
  var serialNumberPart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-barcode'></i> " +
    itemData.info.serie +
    "</b></span>";
  var ammoPart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-gun'></i> " +
    itemData.info.ammo +
    "</b></span>";

  // Weapon Casing Evidence
  var streetPart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-earth-americas'></i> " +
    itemData.info.street +
    "</b></span>";
  var casingPart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-clipboard'></i> " +
    itemData.info.label +
    "</b></span>";
  var ammotypePart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-gun'></i> " +
  ...
  // Default Items
  var weightPart =
    "<span style=\"background-color: #1a1c1c; padding: 5px;\"><b> <i class='fas fa-weight-hanging'></i> " +
    itemData.weight +
    "</b></span>";
  var qualityPart =
    "<span style=\"background-color: #1a1c1c; padding: 5px; margin-left: 5px;\"><b><i class='fas fa-tools'></i></b> " +
    Math.floor(itemData.info.quality) +
    "</span>";
  var descriptionPart =
    '<a style="font-size:13px; display: block; margin-top: 15px;">' +
  ...
  if (itemData != null && itemData.info != "") {
    if (itemData.name == "id_card") {
      ...
      var finalWeapon =
        ammoPart + "<br><br>" + serialNumberPart + "<br><br>" + "</p>";
      $(".item-info-title").html("<p>" + itemData.label + "</p>");
      ...
      $(".item-info-title").html("<p>" + itemData.label + "</p>");
      $(".item-info-description").html(
        "<p style=\"font-size:16px\"><b> <i class='fas fa-weight-hanging'></i> </b>" +
          itemData.weight +
...
(() => {
  ...
  Inventory.itemBox = function (data) {
    ...
    if ($existingItemBox.length > 0) {
      ...
      $existingItemBox
        .find(".item-slot-img-itembox img")
        .attr("src", "images/" + data.item.image)
        .attr("alt", data.item.name);
    } else {
