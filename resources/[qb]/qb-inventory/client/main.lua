...
local function HasItem(items, amount)
    ...
    local isTable = type(items) == 'table'
    local isArray = isTable and table.type(items) == 'array' or false
    local totalItems = isTable and 0 or 1
    local count = 0
    local kvIndex = 2
    ...
    for _, itemData in pairs(PlayerData.items) do
        if itemData then
            if isTable then
                for k, v in pairs(items) do
                    local itemKV = { k, v }
                    if itemData.name == itemKV[kvIndex] and (
                        (amount and itemData.amount >= amount) or
                        (not isArray and itemData.amount >= v) or
                        (not amount and isArray)
                    ) then
                        count += 1
                    end
                end
...
RegisterNetEvent('QBCore:Player:SetPlayerData', function(val)
    ...
    -- HARD GUARANTEE items table exists
...
RegisterNetEvent('qb-inventory:client:UseItem', function(item)
    SendNUIMessage({
        action = 'useItem',
        item = item
    })
