const InventoryContainer = Vue.createApp({
  ...
  methods: {
    ...
    async closeInventory() {
      ...
      try {
        await axios.post("https://ps-inventory/CloseInventory", {
          name: inventoryName,
        });
      } catch (error) {
    ...
    endDrag(event) {
      ...
      if (playerSlotElement) {
        ...
        const isOverInventoryGrid = elementsUnderCursor.some(
          (el) =>
            el.classList.contains("inventory-grid") ||
            el.classList.contains("item-grid"),
        );
    ...
    async handlePurchase(targetSlot, sourceSlot, sourceItem, transferAmount) {
      try {
        const response = await axios.post(
          "https://ps-inventory/AttemptPurchase",
          {
            item: sourceItem,
    ...
    async giveItem(item, quantity) {
      if (item && item.name) {
        ...
        if (playerHasItem) {
          ...
          try {
            const response = await axios.post("https://ps-inventory/GiveItem", {
              item: selectedItem,
              amount: amountToGive,
